...
pathhack = "/home/ismail/Dev/Ego_Twitter"
...
def parse_featname_line(line):
    line = line[(line.find(' ')) + 1:]  # remove first field (just a number)
    split = line.split(';')
    name = ';'.join(split[:-1])  # feature name (list of all features except anonymized feature)
    index = int(split[-1].split(" ")[-1])  # feature index (last number in the line)
    return index, name
...
def load_edges():
    ...
    edge_file = open("%s/edges_combined.txt" % (pathhack,), "r")
...
def universal_feature(feature_index):
    ...
    """
    return len(
        [x for x in network.nodes_iter() if network.nodes[x]['feautures'][feature_index] > 0]) // network.order() == 1
